...
---@param createFunc (fun(): T)|T The function which is called to create a new object, or a Class
...
---@return table<string,ObjectPoolDebugInfo>
...
function ObjectPool.__private:_GetDebugInfo()
	...
	for info, count in pairs(counts) do
		if count > DEBUG_STATS_MIN_COUNT then
			tinsert(debugInfo, info)
		end
	end
	if #debugInfo == 0 then
		tinsert(debugInfo, "<none>")
	else
		sort(debugInfo, function(a, b) return counts[a] > counts[b] end)
		for i, info in ipairs(debugInfo) do
			debugInfo[i] = format("[%d] %s", counts[info], info)
		end
	end
	return {
