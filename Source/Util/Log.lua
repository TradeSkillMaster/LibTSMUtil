...
local private = {
	...
	len = 0,
	argTemp = {},
	logToChat = LibTSMUtil.IsTestVersion(),
...
---@param amount? number The amount to raise the stack level by (defaults to 1)
function Log.RaiseStackLevel(amount)
	assert(amount == nil or amount > 0)
	private.stackLevel = private.stackLevel + (amount or 1)
end

---Lowers the stack level of log messages.
---@param amount? number The amount to lower the stack level by (defaults to 1)
function Log.LowerStackLevel(amount)
	assert(amount == nil or amount > 0)
	private.stackLevel = private.stackLevel - (amount or 1)
end
...
function Log.PrepareFormatArgs(...)
	...
	for i = 1, select("#", ...) do
		...
		private.argTemp[i] = arg
	end
	return Table.UnpackAndWipe(private.argTemp)
end
...
function private.Log(severity, fmtStr, ...)
	...
	local msg = strsplit("\n", format(fmtStr, Log.PrepareFormatArgs(...)))
