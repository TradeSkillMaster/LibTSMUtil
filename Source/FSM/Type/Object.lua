...
function FSMObject:Init(initialState, context)
	...
	for name, obj in pairs(self._stateObjs) do
		for toState in obj:_ToStateIterator() do ---@diagnostic disable-line: invisible
...
function FSMObject:ProcessEvent(event, ...)
	...
	if currentStateObj:_HasEventHandler(event) then ---@diagnostic disable-line: invisible
		self:_Transition(TempTable.Acquire(currentStateObj:_ProcessEvent(event, self._context, ...))) ---@diagnostic disable-line: invisible
...
function FSMObject.__private:_Transition(eventResult)
	...
	while result[1] do
		...
		if self._loggingDisabledCount == 0 then
			Log.RaiseStackLevel(2)
			Log.Info("[%s] %s -> %s", self._name, self._currentState, toState)
			Log.LowerStackLevel(2)
		end
		if not toStateObj or not currentStateObj:_IsTransitionValid(toState) then ---@diagnostic disable-line: invisible
			TempTable.Release(result)
			error("Invalid transition")
		end
		self._inTransition = true
		currentStateObj:_Exit(self._context) ---@diagnostic disable-line: invisible
		self._currentState = toState
		result = TempTable.Acquire(toStateObj:_Enter(self._context, TempTable.UnpackAndRelease(result))) ---@diagnostic disable-line: invisible
