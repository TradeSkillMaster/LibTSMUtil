...
---@generic K
---@param tbl table<K,any> The table to get the keys of
---@param keys K[] The table to insert the keys into
function Table.GetKeys(tbl, keys)
	local index = #keys
...
---@return fun(): K @Iterator with fields: `key`
---@return table
...
function Table.Filter(tbl, func, ...)
	...
	Table.WipeAndDeallocate(private.filterTemp)
end
...
---@generic K, V
---@param tbl table<K,V> The table to copy entries in to
---@param srcTbl table<K,V> The table to copy entries from
function Table.CopyFrom(tbl, srcTbl)
...
---@generic V
---@param tbls V[][] The tables to compare
---@param result V[] The result table
function Table.GetCommonValuesSorted(tbls, result)
	local numTbls = #tbls
	if numTbls == 0 then
		return
	...
	---@cast tbls +{__commonIndex: number}[]
	for _, tbl in ipairs(tbls) do
		assert(tbl.__commonIndex == nil)
		tbl.__commonIndex = 1
	end
	...
	while true do
		...
		if isEqual then
			...
			tinsert(result, tbls[1][tbls[1].__commonIndex])
			for i = 1, numTbls do
				local t = tbls[i]
				t.__commonIndex = t.__commonIndex + 1
			end
		else
...
function Table.GetDiffOrdered(old, new, inserted, removed)
	...
	local result = private.GetDiffOrdered(old, new, inserted, removed)
	if not result then
		wipe(inserted)
		wipe(removed)
	end
	Table.WipeAndDeallocate(private.diffTemp)
	return result
end
...
---@generic V
---@param tbl V[]
---@return fun(): number, V @Iterator with fields: `index`, `value`
---@return V[]
---@return number
...
---@param tbl table
function Table.WipeAndDeallocate(tbl)
	wipe(tbl)
	-- Setting any key (even to nil) causes lua to deallocate the space previously used to store its keys
	tbl.__reset = nil
end
...
function private.KeyIterator(tbl, prevKey)
...
function private.GetDiffOrdered(old, new, inserted, removed)
		...
		if oldValue == new[indexNew] then
			...
			if foundIndexNew then
				...
				for i = indexNew, foundIndexNew - 1 do
					...
					if private.diffTemp[newValue] then
						return false
					end
					private.diffTemp[newValue] = true
					tinsert(inserted, i)
				end
				indexOld = indexOld + 1
				...
				if private.diffTemp[oldValue] then
					return false
				end
				private.diffTemp[oldValue] = true
