...
function ReactiveStateSchema:Extend(name)
	...
	for key, fieldType in self:_FieldIterator() do
		newSchema:_AddField(key, fieldType, self._default[key], self._isOptional[key], self._validateContext[key])
	end
	return newSchema
end
...
function ReactiveStateSchema.__private:_AddField(key, fieldType, default, isOptional, validateContext)
	assert(not self._committed and not self:_HasKey(key))
	if fieldType == "string" or fieldType == "number" or fieldType == "table" then
		assert(validateContext == nil or type(validateContext) == "function")
	elseif fieldType == "boolean" then
		assert(validateContext == nil)
	elseif fieldType == "enum" then
		assert(EnumType.IsType(validateContext))
	elseif fieldType == "class" then
		assert(type(validateContext) == "string" or (type(validateContext) == "table" and validateContext.__isa))
	else
	...
	self._validateContext[key] = validateContext
	self:_ValidateValueForKey(key, default, true)
	return self
end
...
function ReactiveStateSchema:_ValidateValueForKey(key, value, skipValidateFunc)
	...
	if fieldType == "string" or fieldType == "number" or fieldType == "boolean" or fieldType == "table" then
		-- Check the value type
		if type(value) ~= fieldType then
			error("Invalid type for state key ("..tostring(key).."): "..tostring(value))
		end
		-- Validate the value
		local validateContext = not skipValidateFunc and self._validateContext[key] or nil
		if validateContext and not validateContext(value) then
			error("Invalid value for state key ("..tostring(key).."): "..tostring(value))
		end
	elseif fieldType == "enum" then
		-- Check the value type
		if not self._validateContext[key]:HasValue(value) then
			error("Invalid type for state key ("..tostring(key).."): "..tostring(value))
		end
	elseif fieldType == "class" then
		local validateContext = self._validateContext[key]
		local isNameStr = type(validateContext) == "string"
		-- Check the value type
		if isNameStr and value.__class.__name ~= validateContext then
			error("Invalid value for state key ("..tostring(key).."): "..tostring(value))
		elseif not isNameStr and not value:__isa(self._validateContext[key]) then
			error("Invalid value for state key ("..tostring(key).."): "..tostring(value))
		end
	else
		error("Unknown field type: "..tostring(fieldType))
...
---@private
function ReactiveStateSchema:_GetEnumFieldType(key)
	return self._fieldType[key] == "enum" and self._validateContext[key] or nil
end
...
---@private
function ReactiveStateSchema:_GetDefaultValue(key)
